...
static int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "Capricoin+ extended secret key");
    } else
...
static int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "Capricoin+ extended public key");
    } else
...
static int ManageExtKey(CStoredExtKey &sek, std::string &sOptName, std::string &sOptValue, UniValue &result, std::string &sError)
{
    ...
    if (sOptName == "active") {
        if (sOptValue.length() > 0) {
            if (standard::IsStringBoolPositive(sOptValue)) {
...
static UniValue extkey(const JSONRPCRequest &request)
{
    ...
    CHDWallet *const pwallet = GetCapricoinPlusWallet(wallet.get());
    ...
    if (mode == "import") {
        ...
        if (fBip44) {
            ...
            if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                ...
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Import failed - Key must begin with a capricoinplus prefix.");
            }
        }
    ...
    if (mode == "importaccount") {
        ...
        if (request.params.size() > nParamOffset) {
            ...
            if (standard::IsStrOnlyDigits(sVar)) {
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"extkeyaltversion",
                ...
                "If the provided ext_key has a Bitcoin prefix the output will be encoded with a Capricoin+ prefix.\n"
                "If the provided ext_key has a Capricoin+ prefix the output will be encoded with a Bitcoin prefix.\n",
                {
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"getnewextaddress",
                "\nReturns a new Capricoin+ ext address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                ...
                RPCResult{
            "\"address\"              (string) The new capricoinplus extended address\n"
                },
                RPCExamples{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{"getnewstealthaddress",
                "\nReturns a new Capricoin+ stealth address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"label", RPCArg::Type::STR, /* default */ "", "If specified the key is added to the address book."},
            ...
            "           Stealth addresses with prefixes will scan only incoming stealth transactions with a matching prefix."},
                ...
                RPCResult{
            "\"address\"              (string) The new capricoinplus stealth address\n"
                },
                RPCExamples{
...
static UniValue filtertransactions(const JSONRPCRequest &request)
{
    ...
    if (!request.params[0].isNull()) {
        ...
        if (options["from"].isStr()) {
            timeFrom = standard::strToEpoch(options["from"].get_str().c_str());
        } else
        if (options["from"].isNum()) {
            timeFrom = options["from"].get_int64();
        }
        if (options["to"].isStr()) {
            timeTo = standard::strToEpoch(options["to"].get_str().c_str(), true);
        } else
        if (options["to"].isNum()) {
...
static UniValue filteraddresses(const JSONRPCRequest &request)
{
    ...
    UniValue result(UniValue::VARR);
    {
        ...
        for (it = pwallet->mapAddressBook.begin(); it != pwallet->mapAddressBook.end(); ++it) {
            ...
            if (nMatchMode) {
                if (!standard::stringsMatchI(it->second.name, sMatch, nMatchMode-1)) {
                    continue;
                }
            }
...
static UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of capricoinplus in the network\n"
    ...
    CAmount nMoneySupply;
    {
        ...
        rCoinYearReward = Params().GetCoinYearReward() / CENT;
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspentanon",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "", "A json array of capricoinplus addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, /* default */ "", "capricoinplus address"},
                        },
                    },
            ...
            "    \"address\" : \"address\",    (string) the capricoinplus address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValidStealthAddress())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Capricoin+ stealth address: ")+input.get_str());
...
void ReadCoinControlOptions(const UniValue &obj, CHDWallet *pwallet, CCoinControl &coin_control)
{
    if (obj.exists("changeaddress")) {
        ...
        if (!fHaveScript) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid capricoinplus address");
            }
    ...
    if (request.params[0].isArray()) {
        ...
        for (size_t k = 0; k < outputs.size(); ++k) {
            ...
            if (typeOut == OUTPUT_RINGCT
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Capricoin+ stealth address");
            }
...
static std::string SendHelp(CHDWallet *pwallet, OutputTypes typeIn, OutputTypes typeOut)
{
    ...
    rv += std::string(" standard in a") + (typeOut == OUTPUT_RINGCT || typeOut == OUTPUT_CT ? " blinded" : "") + " payment to a given address"
        + (typeOut == OUTPUT_CT ? " in anon standard": "") + ".\n";

    rv += HelpRequiringPassphrase(pwallet);

    rv +=   "\nArguments:\n"
            "1. \"address\"     (string, required) The capricoinplus address to send to.\n"
...
static UniValue sendstandardtoblind(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetCapricoinPlusWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendstandardtoanon(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetCapricoinPlusWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendblindtostandard(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetCapricoinPlusWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendanontostandard(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetCapricoinPlusWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtypeto",
                "\nSend standard to multiple outputs." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"typein", RPCArg::Type::STR, RPCArg::Optional::NO, "standard/blind/anon"},
                    {"typeout", RPCArg::Type::STR, RPCArg::Optional::NO, "standard/blind/anon"},
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of json objects",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::NO, "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The capricoinplus address to send to."},
            ...
            "\"txid\"              (string) The transaction id.\n"
                ...
                RPCExamples{
            HelpExampleCli("sendtypeto", "anon standard \"[{\\\"address\\\":\\\"PbpVcjgYatnkKgveaeqhkeQBFwjqR7jKBR\\\",\\\"amount\\\":0.1}]\"")
                },
            }.ToString());
...
static UniValue createrawstandardtransaction(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetCapricoinPlusWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"createrawstandardtransaction",
                    ...
                    {"outputs", RPCArg::Type::ARR, /* default */ "", "a json array with outputs (key-value pairs).",
                        {
                            {"", RPCArg::Type::OBJ, /* default */ "", "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The capricoinplus address."},
            ...
            "}\n"
                ...
                RPCExamples{
            HelpExampleCli("createrawstandardtransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"address\\\":0.01}\"")
            + HelpExampleCli("createrawstandardtransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"data\\\":\\\"00010203\\\"}\"") +
            "\nAs a JSON-RPC call\n"
            + HelpExampleRpc("createrawstandardtransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", \"{\\\"address\\\":0.01}\"")
            + HelpExampleRpc("createrawstandardtransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", \"{\\\"data\\\":\\\"00010203\\\"}\"")
                },
            }.ToString());
    ...
    rawTx.nVersion = CAPRICOINPLUS_TXN_VERSION;
...
static UniValue fundrawtransactionfrom(const JSONRPCRequest& request)
{
                    ...
                    {"options", RPCArg::Type::OBJ, /* default */ "", "",
                        {
                            ...
                            "                              Those recipients will receive less capricoinplus than you enter in their corresponding amount field.\n"
    ...
    tx.nVersion = CAPRICOINPLUS_TXN_VERSION;
...
{ //  category              name                                actor (function)                argNames
  //  --------------------- ------------------------            -----------------------         ----------
    ...
    //sendstandardtostandard // normal txn
    { "wallet",             "sendstandardtoblind",                  &sendstandardtoblind,               {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },
    { "wallet",             "sendstandardtoanon",                   &sendstandardtoanon,                {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },

    { "wallet",             "sendblindtostandard",                  &sendblindtostandard,               {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },
    ...
    { "wallet",             "sendanontostandard",                   &sendanontostandard,                {"address","amount","comment","comment_to","subtractfeefromamount","narration","ringsize","inputs_per_sig"} },
    ...
    { "rawtransactions",    "createrawstandardtransaction",         &createrawstandardtransaction,      {"inputs","outputs","locktime","replaceable"} },
