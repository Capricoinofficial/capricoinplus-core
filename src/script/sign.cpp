...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    bool fIsP2SH = creator.IsCapricoinPlusVersion()
    ...
    if (creator.IsCapricoinPlusVersion() && !sigdata.witness) {
        sigdata.scriptWitness.stack = result;
    } else  {
...
public:
    ...
    bool is_capricoinplus_tx = false;
    bool IsCapricoinPlusVersion() const override { return is_capricoinplus_tx; }
};
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const std::vector<uint8_t> &amount, const CScript &scriptPubKey)
{
    ...
    extractor_checker.is_capricoinplus_tx = tx.IsCapricoinPlusVersion();
...
bool SignSignature(const SigningProvider &provider, const CTransaction& txFrom, CMutableTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    if (txTo.IsCapricoinPlusVersion()) {
...
class DummySignatureCheckerCapricoinPlus : public DummySignatureChecker
{
// IsCapricoinPlusVersion() must return true to skip stack evaluation
public:
    DummySignatureCheckerCapricoinPlus() : DummySignatureChecker() {}
    bool IsCapricoinPlusVersion() const override { return true; }
};
const DummySignatureCheckerCapricoinPlus DUMMY_CHECKER_CAPRICOINPLUS;

class DummySignatureCreatorCapricoinPlus : public DummySignatureCreator {
public:
    DummySignatureCreatorCapricoinPlus() : DummySignatureCreator(33, 32) {}
    const BaseSignatureChecker& Checker() const override { return DUMMY_CHECKER_CAPRICOINPLUS; }
    bool IsCapricoinPlusVersion() const override { return true; }
};
...
const BaseSignatureCreator& DUMMY_SIGNATURE_CREATOR_CAPRICOINPLUS = DummySignatureCreatorCapricoinPlus();
